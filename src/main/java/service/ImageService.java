...
public class ImageService  {
    ...
    public boolean save(ImageProduct image,int id) {
        if (image == null)
            return false;
        int n = 0;
    ...
    public ImageProduct findByIdImage(int id) {
        ...
        try {
            ...
            while (res.next()) {
                image = new ImageProduct(res.getInt("id"),res.getString("name"),res.getString("url"),res.getString("type"));
            }
        } catch (SQLException e) {
    ...
    public List<ImageProduct> findByObjectId(int id, String type) {
        ...
        try {
            ...
            res = pstmt.executeQuery();
            while (res.next()) {
                images.add(new ImageProduct(res.getInt("id"),res.getString("name"),res.getString("url"),res.getString("type")));
            }
        } catch (SQLException e) {
            System.out.println("Error while fetching image by id and type: " + e.getMessage());
        } finally {
    ...
    public boolean update(ImageProduct image) {
        if (image == null)
            return false;
        int n = 0;
        PreparedStatement pstmt = null;
    ...
    public boolean delete(ImageProduct i) {
        if (i == null)
            return false;
        PreparedStatement pstmt = null;
        int n = 0;
